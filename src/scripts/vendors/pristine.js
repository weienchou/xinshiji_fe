(function() {
    ...
    var lang = {
        ...
        'required-or-check': "This field is required"
    };
    ...
    var SELECTOR = "input:not([type^=hidden]):not([type^=submit]), select, textarea, div";
    ...
    _('pattern', {
        fn: function fn(val, pattern) {
            ...
            console.log(val, pattern, m, ' ==== ');
            return !val || new RegExp(m[1], m[2]).test(val);
        }
    });
    _('required-or-check', {
        fn: function fn(val, pattern) {
            const group = f(this).closest('.form-group');
            const checkbox = group.siblings('.form-group').find('input[type="checkbox"]');
